...
type TransactionReceipt struct {
	...
	BlockNumber       *big.Int  `json:"block_number" comment:"所属区块的区块号"`
	CumulativeGasUsed uint64    `json:"cumulative_gas_used" comment:"区块中从起始位置到此交易消耗的累计Gas量"`
	GasUsed           uint64    `json:"gas_used" comment:"此交易实际消耗的Gas量"`
	ContractAddress   string    `gorm:"size:42" json:"contract_address" comment:"如果是合约部署交易，记录合约地址"`
	Status            uint64    `json:"status" comment:"交易执行状态，1表示成功，0表示失败"`
